app.controller('LocationSelectController', ['$scope', '$rootScope', 'LocationService', 
  ...
  $scope.location_type_value_select = function(index, call_back){
    ...
    console.log("location_type-num", $scope.location_type_num, $scope.name);
    ...
    if($scope.location_types_value[index] == $scope.SELECT){
      console.log("resetting values");
      for(i=index+1; i<= $scope.location_type_num; i++){
        $scope.locations[i] = [];
        $scope.location_types_value[i] = null;
      }
      if(call_back != undefined)
        call_back();
      return;
    ...
    for(i=index+1; i< $scope.location_type_num; i++){
      $scope.locations[i] = [$scope.SELECT];
    }
    $scope.locations[$scope.location_type_num] = [];
    // location_id = $scope.location_maps[index][$scope.location_types_value[index]];
    ...
    LocationService.get_sublocations(location_id, function(response){
        ...
        if(!($scope.location_type_num == index+1)){
          result.unshift({"name" : $scope.SELECT, "id" : $scope.SELECT_ID});
        }
        ...
        if(!($scope.location_type_num == index+1)){
          $scope.location_types_value[index+1] = $scope.SELECT;
        }
        $scope.location_types_value[index+1] = $scope.SELECT;
        ...
        console.log("location_ids", $scope.location_ids);
  ...
  $scope.select_location_type = function(){
    ...
    if(($scope.location_type_num > 0 && $scope.name=="location") || 
      ($scope.location_type_num >= 0 && $scope.name != "location")){
      ...
      LocationService.get_sublocations($rootScope.account.location_id, function(response){
        ...
        // for(i=0; i<result.length; i++){
        (result || []).forEach(function(element, i, result){
          result_names[i] = result[i].name;
          result_ids[i] = result[i].id;
          location_map[result[i].name] = result[i].id
        });
        ...
        if(!($scope.location_type_num == 0)){
          $scope.location_types_value[0] = $scope.SELECT;
        }
